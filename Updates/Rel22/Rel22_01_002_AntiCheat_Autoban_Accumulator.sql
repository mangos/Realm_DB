...
BEGIN
    ...
    SET @cNewDescription = 'AntiCheat_Autoban_Accumulator';

                        -- COMMENT is 150 Characters MAX
    SET @cNewComment = 'Anti-cheat per-account decaying autoban accumulator; ban_count drives escalation tier.';
    ...
    IF (@cCurResult = @cOldResult) THEN    -- Does the current version match the expected version
        ...
        -- Anti-cheat per-account autoban accumulator. Decaying kick-weight
        -- accumulator so spacing offences out still accumulates toward a ban;
        -- ban_count drives the escalation tier. Created only if absent.
